...
export function UnifiedProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUnifiedData = useCallback(async () => {
    ...
    try {
      console.log('📡 [UnifiedContext] Buscando dados unificados...');
      const response = await fetch('/api/unified-financial/optimized', {
        credentials: 'include',
        cache: 'no-cache'
      });
      
      console.log('📡 [UnifiedContext] Resposta recebida:', response.status);
  ...
  useEffect(() => {
    ...
    console.log('🚀 [UnifiedContext] useEffect executado - iniciando fetchUnifiedData');
    console.log('🚀 [UnifiedContext] Estado inicial - loading:', loading, 'data:', data);
    fetchUnifiedData();
  ...
  useEffect(() => {
    const handleCacheInvalidation = (event: CustomEvent) => {
      ...
      if (entity === 'unified-financial-data' || entity === 'transactions') {
        console.log('🔄 [UnifiedContext] Refreshing data due to cache invalidation');
        fetchUnifiedData();
      }
    };

    const handleAccountsUpdated = () => {
      console.log('🔄 [UnifiedContext] Accounts updated event received');
      fetchUnifiedData();
    };
  ...
  const actions = useMemo(() => ({
    ...
    forceRefresh: async () => {
      console.log('🔄 [UnifiedContext] Force refresh triggered');
      setLoading(true);
    ...
    createInstallments: async (installmentData: InstallmentInput) => {
      try {
        ...
        console.log('✅ [UnifiedContext] Parcelamento criado:', result.installments?.length, 'parcelas');
        await fetchUnifiedData();
    ...
    payInstallment: async (installmentId: string, accountId: string) => {
      try {
        ...
        console.log('✅ [UnifiedContext] Parcela paga:', installmentId);
        await fetchUnifiedData();
    ...
    createTransfer: async (fromAccountId: string, toAccountId: string, amount: number, description: string) => {
      try {
        ...
        console.log('✅ [UnifiedContext] Transferência criada');
        await fetchUnifiedData();
    ...
    createSharedExpense: async (expenseData: SharedDebtInput) => {
      try {
        ...
        console.log('✅ [UnifiedContext] Despesa compartilhada criada');
        await fetchUnifiedData();
    ...
    paySharedDebt: async (debtId: string, accountId: string, amount: number) => {
      try {
        ...
        console.log('✅ [UnifiedContext] Dívida paga:', debtId);
        await fetchUnifiedData();
        return result;
      } catch (error) {
    ...
    recalculateBalances: async () => {
      try {
        ...
        console.log('✅ [UnifiedContext] Saldos recalculados');
        await fetchUnifiedData();
    ...
    verifyIntegrity: async () => {
      try {
        ...
        console.log('✅ [UnifiedContext] Integridade verificada');
        return result;
      } catch (error) {
    ...
    createTransaction: async (transactionData: any) => {
      console.log('🔵 [UnifiedContext] createTransaction chamado - usando novo serviço');
      console.log('📤 [UnifiedContext] Dados enviados:', JSON.stringify(transactionData, null, 2));
      try {
        setLoading(true);
        setError(null);

        // ✅ VALIDAR com Zod antes de enviar (validação básica, API fará validação completa)
        console.log('✅ [UnifiedContext] Enviando dados para API');
        ...
        console.log('✅ [UnifiedContext] Transação criada:', result.transaction?.id);
    ...
    updateTransaction: async (id: string, transactionData: any) => {
      try {
        ...
        console.log('✅ [UnifiedContext] Atualizando transação');
        ...
        console.log('✅ [UnifiedContext] Transação atualizada:', id);
    ...
    deleteTransaction: async (id: string) => {
      try {
        ...
        console.log('✅ [UnifiedContext] Transação deletada:', id);
    ...
    createAccount: async (accountData: any) => {
      console.log('🔵 [UnifiedContext] createAccount chamado');
      try {
    ...
    createCreditCard: async (cardData: any) => {
      console.log('🔵 [UnifiedContext] createCreditCard chamado');
      try {
    ...
    createGoal: async (goalData: any) => {
      console.log('🔵 [UnifiedContext] createGoal chamado');
      try {
    ...
    createTrip: async (tripData: any) => {
      console.log('🔵 [UnifiedContext] createTrip chamado');
      try {
    ...
    createBudget: async (budgetData: any) => {
      console.log('🔵 [UnifiedContext] createBudget chamado');
      try {
