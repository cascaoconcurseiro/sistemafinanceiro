...
class OptimizedApiClient {
  ...
  async request<T>(endpoint: string, config: RequestConfig = {}): Promise<T> {
    ...
    if (cache && method === 'GET') {
      ...
      if (cached) {
        console.log('🚀 [OptimizedApiClient] Cache hit:', endpoint);
        return cached;
      }
    }
    ...
    if (this.pendingRequests.has(cacheKey)) {
      console.log('🔄 [OptimizedApiClient] Request already pending:', endpoint);
  ...
  async preload(endpoints: string[]): Promise<void> {
    console.log('🚀 [OptimizedApiClient] Preloading data for', endpoints.length, 'endpoints');
    ...
    console.log('✅ [OptimizedApiClient] Preload completed');
  }
