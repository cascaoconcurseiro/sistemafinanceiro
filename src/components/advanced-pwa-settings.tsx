...
export function AdvancedPWASettings() {
  ...
  const loadSettings = () => {
    try {
      ...
      logError.ui('Failed to load PWA settings:', error);
    }
  };
  ...
  const saveSettings = (newSettings: PWASettings) => {
    try {
      ...
      logError.ui('Failed to save PWA settings:', error);
  ...
  const checkPWAStatus = async () => {
    ...
    if (
      ...
      try {
        ...
        logError.ui('Failed to estimate cache size:', error);
      }
    }
  ...
  const handleNotificationToggle = async (enabled: boolean) => {
    if (enabled) {
      ...
      try {
        ...
        logError.ui('Failed to enable notifications:', error);
  ...
  const clearCache = async () => {
    if ('caches' in window) {
      try {
        ...
        logError.ui('Failed to clear cache:', error);
  ...
  const registerBackgroundSync = () => {
    if (
      ...
      navigator.serviceWorker.ready
        ...
        .catch((error) => {
          logError.ui('Background sync registration failed:', error);
