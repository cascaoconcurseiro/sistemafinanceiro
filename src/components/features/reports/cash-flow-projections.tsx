...
// Função formatCurrency agora é importada de utils
...
export function CashFlowProjections({ onUpdate }: CashFlowProjectionsProps) {
  ...
  const generateProjections = async () => {
    ...
    try {
      ...
      console.log('💰 [CashFlow] Transações para cálculo:', {
        total: transactionsData.length,
        sample: transactionsData.slice(0, 3).map(t => ({ amount: t.amount, type: t.type, date: t.date }))
      });
      ...
      const last6Months = transactionsData.filter((t) => {
      ...
      const incomeTransactions = last6Months.filter((t) => t.type === 'income');
      const expenseTransactions = last6Months.filter((t) => t.type === 'expense');
      
      const monthlyIncome = incomeTransactions.reduce((sum, t) => sum + t.amount, 0) / 6;
      const monthlyExpenses = expenseTransactions.reduce((sum, t) => sum + Math.abs(t.amount), 0) / 6;

      console.log('💰 [CashFlow] Cálculos mensais:', {
        incomeTransactions: incomeTransactions.length,
        expenseTransactions: expenseTransactions.length,
        monthlyIncome,
        monthlyExpenses,
        sampleIncomes: incomeTransactions.slice(0, 3).map(t => t.amount),
        sampleExpenses: expenseTransactions.slice(0, 3).map(t => t.amount)
      });
