...
import { useUnified, useUnifiedFinancial } from '@/contexts/unified-financial-context';
...
const BudgetPerformanceAnalyzer = memo(() => {
  ...
  // Usar dados reais do contexto unificado
  const { data, isLoading: dataLoading } = useUnifiedFinancial();
  
  // Extrair dados do contexto unificado
  ...
  const loadPerformanceData = useOptimizedCallback(
    async () => {
      try {
        ...
        // Usar dados reais de transações e orçamentos
        const now = new Date();
        ...
        // Filtrar transações no período selecionado
        ...
        const categoryData = {
          categories: Object.entries(categoryExpenses).map(([name, amount]) => ({
            name,
            amount
          }))
        };
        ...
        const budgetAccuracy = totalBudgeted > 0 ? Math.min(100, (expenses / totalBudgeted) * 100) : 0;
        
        setMetrics({
          accuracy: budgetAccuracy,
          efficiency: income > 0 ? Math.min(100, ((income - expenses) / income) * 100) : 0,
          savings: income > 0 ? Math.max(0, ((income - expenses) / income) * 100) : 0,
          goalAchievement: 75, // Placeholder - implementar com dados de metas
          categoryOptimization: 80, // Placeholder - implementar análise de categorias
          predictiveAccuracy: 85, // Placeholder - implementar previsões
        });

        // Gerar tendências mensais
        const monthlyData = [];
        for (let i = 0; i < 6; i++) {
          ...
          monthlyData.unshift({
            ...
            variance: monthIncome - monthExpenses,
            efficiency: monthIncome > 0 ? Math.min(100, ((monthIncome - monthExpenses) / monthIncome) * 100) : 0,
          });
        }
        ...
        setCategoryPerformance(
          categoryData.categories.map((c, idx) => {
            ...
            return {
              ...
              budgeted,
              spent: c.amount,
              variance: budgeted - c.amount,
              trend: 'stable' as const,
              score: budgeted > 0 ? Math.min(100, (c.amount / budgeted) * 100) : 0,
    ...
    [timeRange, transactions, budgets],
    300
  ); // Debounce de 300ms
